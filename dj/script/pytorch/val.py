...
def load(ckpt_dir, net):  # 모델 불러오기 함수 (validation용 - optim 불필요)
    ckpt_lst = os.listdir(ckpt_dir)  # 체크포인트 디렉토리의 파일 목록 가져오기
    ckpt_lst.sort()  # 파일 목록 정렬
    dict_model = torch.load(os.path.join(ckpt_dir, ckpt_lst[-1]), map_location=device)  # 가장 최근 체크포인트 로드 (절대 경로)
    
    net.load_state_dict(dict_model['net'])  # 네트워크 가중치 로드
    return net  # 로드된 네트워크 반환
## MNIST 데이터 불러오기
...
dataset = datasets.MNIST(root=data_dir, train=False, download=True, transform=transform)  # MNIST 테스트 데이터셋 로드
print(f"MNIST test dataset loaded successfully! Total samples: {len(dataset)}")  # 데이터 로딩 완료 메시지

loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=0)  # 데이터 로더 생성
...
## 학습된 모델 불러오기
print("Loading trained model...")  # 모델 로딩 시작 메시지
net = load(ckpt_dir, net)  # 체크포인트에서 모델 로드
print("Model loaded successfully!")  # 모델 로딩 완료 메시지
...
with torch.no_grad():
    ...
    # Validation 종료 후 평균 손실과 정확도 출력
    avg_loss = np.mean(loss_arr)  # 평균 손실 계산
    avg_acc = np.mean(acc_arr)  # 평균 정확도 계산
    print('=' * 50)  # 구분선
    print(f'Validation Results:')  # Validation 결과 헤더
    print(f'Loss: {avg_loss:.6f}')  # 평균 손실 출력
    print(f'Accuracy: {avg_acc:.4f} ({avg_acc*100:.2f}%)')  # 평균 정확도 출력
    print('=' * 50)  # 구분선